...
#' @export
#' @seealso [best_match()]
score_multiple <- function(input, candidates, .fn = lev_ratio, ..., decreasing = TRUE) {
...
#' calculated by `.fn`.
#'
#' @inheritParams score_multiple
#' @return A string
#' @seealso [score_multiple()]
#' @examples
#' best_match("bilbo", c("frodo", "gandalf", "legolas"))
#' @export
best_match <- function(input, candidates, .fn = lev_ratio, ..., decreasing = TRUE) {
  scores <- score_multiple(input = input, candidates = candidates, .fn = .fn, ..., decreasing = decreasing)
  names(scores)[[1]]
}
